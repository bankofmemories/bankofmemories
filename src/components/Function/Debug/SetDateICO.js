...
import { setStartDateICO } from '../../../utils/debug/setStartDateICO';
...
class SetDateICO extends Component {
  constructor(props) {
    super(props);
  ...
  async setStartDate() {
    ...
    await setStartDateICO(date).then(async (result) => {
  ...
  render() {
    return (
        <div>
          ...
          <Row>
            <Col md={{ size: '3' }}>
              <Button color="danger" onClick={() => this.finish()} >Finish Minting</Button>
            </Col>
          </Row>
        </div>
    );
